...
void main() {
  ...
  Future useTodoTaskController(
    FutureOr Function(TodoTaskController todoController) test, {
    TodoTask? defaultValue,
  }) async {
    final todoController = Get.put(TodoTaskController(defaultValue));
    await test(todoController);
    Get.delete<TodoTaskController>();
  }

  group('Todo Task Test : ', () {

    test('Insert Multiple Dummy TodoTask', () async {
      for (var i = 1; i < 14; i++) {
        await useTodoTaskController((todoController) async {
          ...
          expect(
              listContains(controller.inProgressTodoTaskList, newTask), isTrue,
              reason: "newTask must be in inProgressTodoTaskList");
          //index check (0)
          expect(controller.inProgressTodoTaskList[0].id, newTask.id,
              reason:
                  "newTask.id must be first item in inProgressTodoTaskList");
        });
      }
    });
    ...
    test('Toggle TodoTask Status', () async {
      ...
      for (var i = 1; i < controller.inProgressTodoTaskList.length; i += 2) {
        ...
        expect(
            listContains(controller.inProgressTodoTaskList, todoTask), isFalse,
            reason: "todoTask must not be in inProgressTodoTaskList");
        expect(todoTask, isNotNull, reason: "todoTask must not be null");
    ...
    test('Reorder TodoTask', () async {
      ...
      reorderTest(
        ...
        expect(list[targetIndex].id, todoTask.id,
            reason: "list[$targetIndex].id must be todoTask.id");
        ...
        if (prevTask != null) {
          expect(prevTask.nextId, nextTask?.id,
              reason: "prevTask.nextId must be nextTask?.id");
        }
        if (nextTask != null) {
          expect(nextTask.prevId, prevTask?.id,
              reason: "nextTask.prevId must be prevTask?.id");
        }
      }
      ...
      updateTest(List<TodoTask> list, int index) async {
        ...
        await useTodoTaskController((todoController) async {
          ...
          if (newTodoTask.status == TodoTaskStatus.completed) {
            expect(listContains(controller.inProgressTodoTaskList, newTodoTask),
                isFalse,
                reason: "newTodoTask must not be in inProgressTodoTaskList");
            expect(listContains(controller.completedTodoTaskList, newTodoTask),
                isTrue,
          ...
          expect(list[index].description, newTodoTask.description,
              reason:
                  "list[$index].description must be newTodoTask.description");
